...
def fig_complementarity(path: Path, summary: dict) -> None:
    ...
    feats = ["plateau (R)", "PBO", "DSR", "plateau+stat"]
    colors = ["#9aa6b2", "#e0a458", "#c0392b", "#1f3b73"]
    x = np.arange(len(labels)); w = 0.2
    fig, ax = plt.subplots(figsize=(5.2, 3.2))
    for k, (f, c) in enumerate(zip(feats, colors)):
        vals = [comb[(comb.label == lb) & (comb.features == f)]["auc"].values[0] for lb in labels]
        ax.bar(x + (k - 1.5) * w, vals, w, label=f, color=c)
    ax.axhline(0.5, color="k", lw=0.8, ls=":")
    ax.set_xticks(x, ["no-edge", "fragile", "OOS-loss"])
    ax.set_ylim(0.45, 0.85)
    ax.set_ylabel("cross-validated AUC")
    ax.set_title("Combining geometry with statistics beats either alone")
...
def fig_selection(path: Path, summary: dict, df1: pd.DataFrame) -> None:
    ...
    axL.set_xlabel("surrogate curvature tercile (sharpness of the naive optimum)")
...
def main() -> None:
    ...
    d1 = summary["dim1"]
    d1["bandwidth_sweep"] = summary["bandwidth_sweep"]
